...
(function (globals) {
  'use strict';
  globals.portfolio = angular.module("portfolio", ["ui.router", "pascalprecht.translate", "ngSanitize", "vcRecaptcha"]);

  /*###################################################################################################################
  # APP CONFIGURATION
  ###################################################################################################################*/
  portfolio.config(["$stateProvider", "$urlRouterProvider", "$locationProvider", "$translateProvider", "vcRecaptchaServiceProvider", function($stateProvider, $urlRouterProvider, $locationProvider, $translateProvider, vcRecaptchaServiceProvider) {
  ...
  portfolio.run(['$rootScope', function ($rootScope) {
    ...
    $rootScope.imgPath  = $rootScope.rootPath + "img/";                     /* Images folder path */
    $rootScope.jsPath   = $rootScope.rootPath + "js/";                      /* JavaScript folder path */
    $rootScope.cssPath  = $rootScope.rootPath + "css/";                     /* CSS folder path */
  }]);
}( (1,eval)('this') ));
